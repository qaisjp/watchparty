...
class Docker extends base_1.VMManager {
    constructor() {
        ...
        docker run -d --rm --name=${name} --net=host --memory="2g" --cpus="2" -v /etc/letsencrypt:/etc/letsencrypt -l vbrowser -l index=$INDEX --log-opt max-size=1g --shm-size=1g --cap-add="SYS_ADMIN" -e NEKO_KEY="/etc/letsencrypt/live/${gatewayHost}/privkey.pem" -e NEKO_CERT="/etc/letsencrypt/live/${gatewayHost}/fullchain.pem" -e DISPLAY=":$INDEX.0" -e NEKO_SCREEN="1280x720@30" -e NEKO_PASSWORD=${name} -e NEKO_PASSWORD_ADMIN=${name} -e NEKO_BIND=":$PORT" -e NEKO_EPR=":$UDP_START-$UDP_END" ${utils_1.imageName}
        ...
        this.terminateVM = (id) => __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                ssh_exec_1.default(`docker rm -f ${id}`, sshConfig, (err, stdout) => {
                    if (err) {
                        return reject(err);
                    }
                    resolve();
        ...
        this.getVM = (id) => __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                ssh_exec_1.default(`docker inspect ${id}`, sshConfig, (err, stdout) => {
                    if (err) {
                        return reject(err);
                    }
                    let data = null;
        ...
        this.listVMs = (filter) => __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
                ssh_exec_1.default(`docker inspect $(docker ps --filter label=${filter} --quiet --no-trunc) || true`, sshConfig, (err, stdout) => {
